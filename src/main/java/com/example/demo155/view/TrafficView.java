...
import com.example.demo155.Model.Vehicle;
...
public class TrafficView {
    ...
    private void addVehicle(String road) {
        Random random = new Random();
        VehicleType type = VehicleType.values()[random.nextInt(VehicleType.values().length)];
        Node sprite;
        int x, y;
        int offset = vehicles.stream().filter(v -> v.getRoad().equals(road)).toList().size() * 80; // Her araç için 80 piksel mesafe

        switch (road) {
            case "North":
                x = 300;
                y = -50 - offset; // Yukarıda sıralı, ekran dışı başlangıç
                break;
            case "South":
                x = 300;
                y = 650 + offset; // Aşağıda sıralı, ekran dışı başlangıç
                break;
            case "West":
                x = -50 - offset; // Solda sıralı, ekran dışı başlangıç
                y = 350;
                break;
            case "East":
                x = 650 + offset; // Sağda sıralı, ekran dışı başlangıç
                y = 250;
                break;
            default:
                x = 300;
        ...
        Vehicle vehicle = new Vehicle(x, y, type, sprite, controller);
    ...
    private void updateVehicles() {
        for (Vehicle vehicle : new ArrayList<>(vehicles)) {
            vehicle.update();
            if (vehicle.getX() < -50 || vehicle.getX() > 650 || vehicle.getY() < -50 || vehicle.getY() > 650) {
                roadPane.getChildren().remove(vehicle.getSprite());
    ...
    private GridPane createControlPane() {
        ...
        resetBtn.setOnAction(e -> {
            ...
            vehicleTimer.stop();
            vehicles.clear();
            roadPane.getChildren().removeIf(node -> node instanceof TaxiView || node instanceof TruckView || node instanceof BusView);
            updateVehicleLabels();
        });
        ...
        assignVehiclesBtn.setOnAction(e -> {
            ...
            vehicles.clear();
            roadPane.getChildren().removeIf(node -> node instanceof TaxiView || node instanceof TruckView || node instanceof BusView);
            for (String road : new String[]{"North", "West", "South", "East"}) {
                int count = controller.getVehicleDistribution().getOrDefault(road, 0);
